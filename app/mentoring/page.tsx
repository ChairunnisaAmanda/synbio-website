import Image from "next/image"
import Link from "next/link"
import { ArrowRight } from "lucide-react"

export default function MentoringPage() {
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center py-16">
      <h1 className="mb-8 text-4xl font-bold tracking-tight text-center text-[#20544B] md:text-5xl">
        Mentoring Programs
      </h1>

      <div className="relative w-full max-w-2xl h-64 md:h-96 mb-8 rounded-lg overflow-hidden">
        <Image
          src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/home-mentoring-YBwVvD9r7DcAHMFb7vGPgcrd9vMWga.png"
          alt="Mentoring"
          fill
          className="object-cover"
        />
      </div>

      <div className="text-center max-w-2xl">
        <h2 className="mb-6 text-3xl font-bold text-[#20544B]">Coming Soon</h2>
        <p className="mb-8 text-xl text-gray-600">
          We're working on our mentoring program to connect experts with aspiring scientists and researchers. Stay tuned
          for updates!
        </p>
        <Link
          href="/"
          className="inline-flex items-center px-6 py-3 font-medium text-[#20544B] transition-colors rounded-md bg-[#F2D668] hover:bg-opacity-90"
        >
          Back to Home
          <ArrowRight className="w-4 h-4 ml-2" />
        </Link>
      </div>
    </div>
  )
}
